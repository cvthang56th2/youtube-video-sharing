      ...
      <div className='flex flex-wrap -mx-3 mt-4'>
        {videosMemo.map((video: VideoType, vIndex: number) => (
          <div className='w-full md:w-1/2 xl:w-1/3 px-3 mb-8' key={`video-${vIndex}`}>
            <div className='last:mb-0 border-2 rounded-xl h-full flex flex-col group hover:shadow-xl cursor-pointer transition-all duration-250 ease-linear' key={`video-${vIndex}`} onClick={() => watchVideo(video.id)}>
