        ...
        {videosMemo.map((video: VideoType, vIndex: number) => (
          <div className='w-full md:w-1/2 xl:w-1/3 px-3 mb-8' key={`video-${vIndex}`}>
            ...
            <div className='last:mb-0 border-2 rounded-xl h-full group hover:shadow-xl cursor-pointer transition-all duration-250 ease-linear relative' key={`video-${vIndex}`}>
              {showActions && (
                <>
                  <span className={['absolute top-1 left-1 text-white z-10 p-1 text-sm rounded-full pointer-events-none', video.isPrivate ? 'bg-gray-400' : 'bg-green-400'].join(' ')}>{video.isPrivate ? 'Private' : 'Public'}</span>
                  <Confirm wrapClasses='absolute top-1 right-1 z-10' onYes={() => changeVideoStatus(video)}>
                    <button className='btn btn-blue text-sm'>Change To {video.isPrivate ? 'Public' : 'Private'}</button>
                  </Confirm>
                </>
              )}
              <div className='h-full flex flex-col' onClick={() => watchVideo(video.id)}>
