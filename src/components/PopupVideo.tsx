...
const PopupVideo = ({ isShow, close, videoId }: PropsType) => {
  ...
  const preventEvents = (event: { preventDefault: () => void; stopPropagation: () => void }) => {
  ...
  return (
    <>
      <div className={["w-screen h-screen fixed inset-0 z-10 overflow-y-auto flex items-center justify-center", isShow ? 'bg-[rgba(0,0,0,0.2)]' : 'pointer-events-none'].join(' ')} onClick={() => close()}>
